...
|------|----------|---------|
| `diff_drive_controller/odom.csv` | Одометрия от дифференциального привода (оценка положения и скоростей на основе энкодеров колёс) | 50 Гц |
| `gps/fix.csv` | Данные GPS-приёмника с абсолютными координатами в формате широта/долгота | 6.7 Гц |
| `imu/data.csv` | Данные инерциального измерительного модуля (IMU), включая ориентацию (кватернион), угловые скорости и линейные ускорения | 187 Гц |
| `ground_truth/gz_odometry.csv` | Истинная траектория робота из симулятора (ground truth) | 50 Гц |
...
**Результаты анализа частоты датчиков:**

| Датчик | Количество измерений | Средний интервал | Частота | NaN значения | Выбросы |
|--------|---------------------|-------------------|---------|--------------|---------|
| Одометрия | 6164 | 20.000 мс | 50.0 Гц | 0 | 0 из 6163 (0.00%) |
| IMU | 23692 | 5.175 мс | 193.2 Гц | 0 | 800 из 23691 (3.38%) |
| GPS | 1227 | 173.773 мс | 5.8 Гц | 1227 | 48 из 1226 (3.92%) |
| Ground Truth | 6232 | 20.000 мс | 50.0 Гц | 0 | 0 из 6231 (0.00%) |
...
Также построены графики угловой скорости wz и линейной скорости vx во времени из одометрии и IMU.
...
**Улучшение относительно одометрии:**
- По X: **-1.9%**
- По Y: **-0.0%**
- По θ: **-21.2%**
